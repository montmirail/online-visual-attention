...
MOT.trialSeed = []; //stores what seed was used for each trial
MOT.drawCounter = []; //stores how many frames were drawn per trial
...
MOT.trialStart = []; //stores start time of each trial
MOT.response = [];  //stores subject's responses per trial
MOT.correct = []; //stores if subject was correct per trial
MOT.rt = []; //response time per trial
...
MOT.dotMovAng = []; //stores current angle of motion for each dot (updated at each frame)
MOT.probeTracked = []; //store whether the trial asked if a dot that needed to be attended to (blue) was the dot that was queried about at the end of a trial
MOT.probedDot = []; //store the identity of the probed dot (the one asked about at the end of the trial)
